...
DirectWrite::DirectWrite()
{
	...
	(
		DWRITE_FACTORY_TYPE_SHARED, //D3D와 호환을 하겠다
	...
	assert(SUCCEEDED(hr));
	hr = factory->CreateDevice(dxgiDevice, &device); //2D Factory 사용할려면 BGRA
	//3D에서 설정후 넘겨줘야함
	assert(SUCCEEDED(hr));
...
void DirectWrite::CreateBackBuffer()
{
	...
	hr = deviceContext->CreateSolidColorBrush(D2D1::ColorF(0, 0, 0, 1), &brush);
	assert(SUCCEEDED(hr));

	writeFactory->CreateTextFormat
	(
		L"돋움체", NULL, DWRITE_FONT_WEIGHT_SEMI_BOLD, DWRITE_FONT_STYLE_NORMAL,
